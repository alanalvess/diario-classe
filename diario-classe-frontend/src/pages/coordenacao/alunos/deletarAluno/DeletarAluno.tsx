...
function DeletarAluno({isOpen, onClose, alunoSelecionado, aoDeletar}: DeletarAlunoProps) {
  ...
  async function excluirAluno() {
    setIsLoading(true);
    try {
      await deletar(`/alunos/${alunoSelecionado.id}`, {
        headers: {Authorization: `Bearer ${usuario.token}`},
      });
      ToastAlerta('Aluno excluido com sucesso', Toast.Success);
      aoDeletar(alunoSelecionado.id);
      onClose();
    } catch (error) {
      if (error instanceof Error) return;
      ToastAlerta('Erro ao deletar aluno', Toast.Error);
    } finally {
      setIsLoading(false);
    }
  }
          ...
          <div className="text-center lg:text-left">
            ...
            <p className="font-normal text-gray-700 dark:text-gray-500">
              ...
              <p className="mb-2">
                ...
                <span className="italic">{alunoSelecionado.matricula}</span>
              </p>
              <p className="mb-2">
                <span className="font-bold">Turma: </span>
                <span className="italic">{alunoSelecionado.turmaNome}</span>
              </p>
            </p>
