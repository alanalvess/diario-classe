...
function DeletarTurma({isOpen, onClose, turmaSelecionada, aoDeletar}: DeletarTurmaProps) {
  ...
  async function excluirTurma() {
    setIsLoading(true);
    try {
      await deletar(`/turmas/${turmaSelecionada.id}`, {
        headers: {Authorization: `Bearer ${usuario.token}`},
      });
      ToastAlerta('Turma excluida com sucesso', Toast.Success);
      aoDeletar(turmaSelecionada.id);
      onClose();
    } catch (error) {
      if (error instanceof Error) return;
      ToastAlerta('Erro ao excluir turma', Toast.Error);
    } finally {
      setIsLoading(false);
    }
  }
        ...
        <Card className="max-w-sm mx-auto lg:gap-10" imgSrc={DeleteImg} horizontal>
          <div className="text-center lg:text-left">
            ...
            <p className="font-normal text-gray-700 dark:text-gray-500">
              ...
              <p className="mb-2">
                <span className="font-bold">Ano Letivo: </span>
                <span className="italic">{turmaSelecionada.anoLetivo}</span>
              </p>
            </p>
