...
function DeletarAvaliacao({isOpen, onClose, avaliacaoSelecionada, aoDeletar}: DeletarAvaliacaoProps) {
  ...
  async function excluirAvaliacao() {
    setIsLoading(true);
    try {
      await deletar(`/avaliacoes/${avaliacaoSelecionada.id}`, {
        headers: {Authorization: `Bearer ${usuario.token}`},
      });
      ToastAlerta('Avaliação excluida com sucesso', Toast.Success);
      aoDeletar(avaliacaoSelecionada.id);
      onClose();
    } catch (error) {
        ...
        <Card className="max-w-sm mx-auto lg:gap-10" imgSrc={DeleteImg} horizontal>
          <div className="text-center lg:text-left">
            ...
            <p className="font-normal text-gray-700 dark:text-gray-500">
              ...
              <p className="mb-2">
                <span className="font-bold">Bimestre: </span>
                <span className="italic">{avaliacaoSelecionada.bimestre}</span>
              </p>
              <p className="mb-2">
                <span className="font-bold">Peso: </span>
                <span className="italic">{avaliacaoSelecionada.peso}</span>
              </p>
            </p>
